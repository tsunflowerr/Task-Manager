...
import { AlignLeft, Calendar, Plus, PlusCircle, Save, X, Flag, CheckCircle } from "lucide-react";
...
const TaskModal = ({isOpen, onClose, taskToEidt, onSave,onLogout}) => {
    ...
    const handleSubmit = useCallback(async(e) => {
        ...
        try {
            ...
            console.error(err)
            setError(err.message || "An unexpected error occurr")
        }
        finally {
            setLoading(false)
        }
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center p-4">
            <div className="bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {error && <div className="text-sm text-red-600 bg-red-50 p-3 rounded-lg border bor-red-100">{error}</div>}
                    <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="dueDate" required min={today} value={taskData.dueDate} onChange={handleChange} className={baseControlClasses}></input>
                        </div>
                    </div>
                    <div>
                        <label className="flex items-center gap-1 text-sm font-medium text-gray-700 mb-1">
                    ...
                    className="w-full bg-gradient-to-r from-fuchsia-500 to-purple-600 text-white font-medium py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50 hover:shadow-md transition-colors duration-200">
