...
const Profile = ({setCurrentUser, onLogout}) => {
    ...
    const changePassword = async(e) =>{
        ...
        try {
            const token = localStorage.getItem('token')
            const {data} = await axios.put(`${API_URL}/api/user/password`, {currentPassword: password.current, newPassword:password.new}, {headers:{Authorization:`Bearer ${token}`}})
            if(data.success) {
            ...
            <div className="max-w-4xl mx-auto p-6">
                ...
                <div className="grid md:grid-cols-2 gap-8"> 
                    <section className={SECTION_WRAPPER}>
                        <div className="flex iteams-center gap-2 mb-6">
                            <UserCircle className="text-purple-500 w-5 h-5" />
                            <h2 className="text-xl font-semibold text-gray-800">Personal Infomation</h2>
                        </div>
                        ...
                        <form onSubmit={changePassword} className="space-y-4">
                            {securityFields.map(({name, placeholder}) =>(
                                <div key={name} className={INPUT_WRAPPER}>
                                    ...
                                    <input type = "password" placeholder={placeholder} value={password[name]} onChange={(e) => setPasswords({...password, [name]: e.target.value})}
                                    className="w-full focus:outline-none text-s" required/>
                                </div>
                            ))}
                            <button className={FULL_BUTTON}>
